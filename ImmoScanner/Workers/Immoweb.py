...
class Immoweb(RealEstateWorker):
    ...
    def fill_empty_fields(self, real_estate_research: RealEstateResearch):
        if real_estate_research.louer_acheter is None:
            real_estate_research.louer_acheter = "a-vendre"

        if real_estate_research.type_bien is None:
            real_estate_research.type_bien = "maison"

        if real_estate_research.pays is None:
            real_estate_research.pays = "Belgique"
    ...
    def get_findings(self, real_estate_research: RealEstateResearch):
        ...
        url = self.creation_url(real_estate_research)
        logging.info(url)
        ...
        try:
            ...
            for page in range(1, nombre_pages + 1):
                url = self.creation_url(real_estate_research, page)
                soup = self.get_soupe(url)
    ...
    def creation_url(self, real_estate_research: RealEstateResearch, page = 1):
        if page != 1:
